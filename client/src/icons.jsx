...
export const Icon = {
  ...
  building: (p) => <S {...p}><rect x="5" y="3" width="14" height="18" rx="1.5" /><path d="M9 7h.01M12 7h.01M15 7h.01M9 11h.01M12 11h.01M15 11h.01M10 21v-4h4v4" /></S>,
  list: (p) => <S {...p}><path d="M8 6h12M8 12h12M8 18h12M4 6h.01M4 12h.01M4 18h.01" /></S>,
  arrow: (p) => <S {...p}><path d="M5 12h14M13 6l6 6-6 6" /></S>,
  link: (p) => <S {...p}><path d="M9 12h6M10 8H8a4 4 0 0 0 0 8h2M14 8h2a4 4 0 0 1 0 8h-2" /></S>,
  users: (p) => <S {...p}><circle cx="9" cy="8" r="3" /><path d="M3 20a6 6 0 0 1 12 0M16 5.5a3 3 0 0 1 0 5.8M21 20a6 6 0 0 0-4-5.6" /></S>,
  present: (p) => <S {...p}><rect x="3" y="4" width="18" height="12" rx="2" /><path d="M8 20h8M12 16v4" /></S>,
};

export const DeviceIcon = { lock: Icon.lock, camera: Icon.camera, light: Icon.bulb, sensor: Icon.sensor };

// SpaceLord brand mark — a keyhole (you hold the key to the space) cut into a
// dark tile. Colors come from CSS so it themes cleanly.
export function Logo({ size = 32 }) {
  return (
    <svg className="brandmark" width={size} height={size} viewBox="0 0 40 40" role="img" aria-label="SpaceLord">
      <rect className="bm-tile" x="1" y="1" width="38" height="38" rx="11" />
      <circle className="bm-key" cx="20" cy="16.4" r="5.3" />
      <path className="bm-key" d="M17.3 19.4 L22.7 19.4 L24.1 30.4 Q24.2 31.2 23.4 31.2 L16.6 31.2 Q15.8 31.2 15.9 30.4 Z" />
    </svg>
  );
}
